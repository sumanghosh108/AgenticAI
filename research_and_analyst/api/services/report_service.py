"""
Report Service — Business logic layer for the report generation pipeline.
"""
...
from research_and_analyst.workflows.report_generator_workflows import AutonomousReportGenerator
...
class ReportService:
    ...
    def __init__(self):
        ...
        # Share memory between agents/components
        self.reporter.memory = _shared_memory

        # Build execution graph for the report workflow
    ...
    def submit_feedback(self, thread_id: str, feedback: str):
        ...
        try:
            ...
            # Update state in the workflow graph
    ...
    def get_report_status(self, thread_id: str):
        ...
        try:
            ...
            if final_report:
                ...
                # Save in each format independently
                file_md = file_docx = file_pdf = None

                try:
                    file_md = self.reporter.save_report(final_report, topic, "md")
                except Exception as save_err:
                    self.logger.warning("Failed to save MD", error=str(save_err))

                try:
                    file_docx = self.reporter.save_report(final_report, topic, "docx")
                except Exception as save_err:
                    self.logger.warning("Failed to save DOCX", error=str(save_err))

                try:
                    file_pdf = self.reporter.save_report(final_report, topic, "pdf")
                except Exception as save_err:
                    self.logger.warning("Failed to save PDF", error=str(save_err))
