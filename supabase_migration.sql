...
-- ─────────────────────────────────────────────
-- 7. User Frequency Tracking (one row per user)
-- ─────────────────────────────────────────────
...
DROP POLICY IF EXISTS "service_role_user_frequency" ON user_frequency;
CREATE POLICY "service_role_user_frequency" ON user_frequency FOR ALL USING (true) WITH CHECK (true);

-- 7a. Atomic login count increment (upsert — one row per user)
...
-- 7b. Atomic report generate count increment
...
-- 7c. Atomic download count increment
...
-- 7d. Get user frequency stats (read-only)
...
DROP TRIGGER IF EXISTS trg_user_frequency_updated_at ON user_frequency;
